...
const WHATSAPP_SELLER_NUMBER = process.env.WHATSAPP_SELLER_NUMBER || "";
...
function sanitizeOrderPayload(payload) {
  ...
  return {
    ...
    payment: {
      method: String(payload?.payment?.method || "Airtel Money / Moov Money").trim(),
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`boutique-pro running on http://localhost:${PORT}`);
  if (lanIp) {
    // eslint-disable-next-line no-console
